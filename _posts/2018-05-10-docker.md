...
安装最新版的mysql：同时设置容器名字，暴露端口，设置密码，后台启动，重启策略，远程镜像名字（不带版本号默认最新版）

`docker run --name mysql -p 3306:3306 -e MYSQL_ROOT_PASSWORD=你的默认密码 -d --restart=unless-stopped mysql:5.6.35`
...
```
yum install java-1.8.0-openjdk  java-1.8.0-openjdk-devel # 新机器，发现没装jdk
./gradlew  build
docker build -t hi .
``` 
